<script lang="ts">
	...
	import { Upload, Camera, X, CloudArrowUp } from 'phosphor-svelte';
	...
	let { currentImage = null, disabled = false, label = 'Profile Picture', id = 'profile-picture' }: Props = $props();
...
<div class="grid w-full gap-3">
	...
	<div class="flex w-full flex-row items-center justify-between gap-3">
		...
		<div
			class="relative mx-auto flex h-32 w-32 items-center justify-center rounded-full border-2 border-dashed border-primary/30 p-1 transition-colors
		...
		>
			...
			{:else if getPreviewUrl()}
				...
				>
					<Camera size={24} weight="bold" class="text-white" />
				</div>
			{:else}
				<div class="flex flex-col items-center gap-2 text-muted-foreground">
					<Camera size={32} weight="duotone" />
					<span class="text-center text-xs">Drop image or click to upload</span>
				</div>
			{/if}
		...
		>
			...
			{currentImage ? 'Change Profile Picture' : 'Choose Profile Picture'}
		</Button>
	</div>
</div>
